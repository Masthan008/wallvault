...
export default function AdminReviewsPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-[#a1a1aa] mt-1">
            Monitor, moderate, and remove misbehaving user comments or inappropriate reviews across all wallpapers.
          </p>
        </div>
      </div>
      ...
      {/* ── Controls & Search ───────────────────────────────── */}
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-center bg-white/[0.02] p-4 rounded-2xl border border-white/[0.06]">
        <div className="relative w-full sm:w-80">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-black/40 border border-white/10 rounded-xl text-xs text-white placeholder-[#71717a] focus:outline-none focus:border-cyan-500/50 transition-colors"
          />
        </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-white/[0.01] rounded-2xl border border-white/[0.05]">
          <ShieldAlert className="w-10 h-10 text-[#52525b] mb-3" />
        ...
        <div className="grid gap-4">
          {filteredReviews.map((item) => (
            <motion.div
              ...
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              className="p-5 rounded-2xl glass-morphism border border-white/[0.06] flex flex-col md:flex-row items-start md:items-center justify-between gap-4"
            >
              <div className="flex items-start space-x-3.5 flex-1 min-w-0">
                <div className="w-10 h-10 rounded-full bg-cyan-500/10 border border-cyan-500/20 overflow-hidden shrink-0 flex items-center justify-center text-cyan-400 font-bold text-sm">
                  {item.userAvatar ? (
                ...
                <div className="flex-1 min-w-0 space-y-1">
                  <div className="flex items-center space-x-2 flex-wrap">
                    ...
                    {item.rating > 0 && (
                      <span className="inline-flex items-center text-[10px] font-bold text-amber-400 bg-amber-400/10 px-2 py-0.5 rounded-full border border-amber-400/20">
                  ...
                  <div className="flex items-center space-x-3 text-[10px] text-[#71717a]">
                    <span>Wallpaper ID: <code className="text-cyan-400/80">{item.wallpaperId}</code></span>
                    <span>•</span>
